...
void adventurerEffect(int drawntreasure, struct gameState *state, int cardDrawn, int temphand) {
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus) {
    ...
    switch (card) {
        case adventurer:
            adventurerEffect(drawntreasure, state);
            return 0;
        ...
        case council_room:
            //+4 Cards
            for (i = 0; i < 4; i++) {
               drawCard(currentPlayer, state, cardDrawn, tempHand);
            }
